...
logger = log.getLogger("NeoManoOpenModule")


class NeoManoOpenModule(Module):
    ...
    PARAMETER_DEFINITION = [
        ...
        {
            'name': 'neutral_hand_time',
            'displayname': 'Time to achieve Neutral Position',
            'description': '',
            'type': float,
            'unit': 's',
            'default': 8.0,
    ...
    def __init__(self):
        ...
        super(NeoManoOpenModule, self).__init__()
        self.state = None
    ...
    def initGui(self):
        ...
        super(NeoManoOpenModule, self).initGui()
    ...
    def start(self):
        ...
        if len(streams) < 1:
            self.setStatus(Module.Status.STOPPED)
            logger.error(
                f"Could not start {self.MODULE_NAME} because of "
                f"missing stream: {globals.STREAM_NAME_CLASSIFIED_SIGNAL}")
            return
            ...
            if in_sample is not None:
                ...
                if relevant_exo_state != self.last_relevant_exo_state:
                    ...
                    if relevant_exo_state == enums.ExoState.OPEN:
                        ...
                        self.state_machine_state = self.STATE_NEUTRAL_HAND
                        self.state_machine_state_change_time = clock()
    ...
    def state_machine(self):
        ...
        while True:
            ...
            # neutral position
            elif self.state_machine_state == self.STATE_NEUTRAL_HAND:

                if time_since_state_change < (self.getParameter('neutral_hand_time') - 2.0):
                    self.send_command_to_exo(0)
                elif (time_since_state_change < self.getParameter('neutral_hand_time')) and \
                        (time_since_state_change > (self.getParameter('neutral_hand_time')-2.0)):
                    self.send_command_to_exo(1)
                elif time_since_state_change > self.getParameter('neutral_hand_time'):
                    self.state_machine_state = self.STATE_STOP
                    self.state_machine_state_change_time = clock()
