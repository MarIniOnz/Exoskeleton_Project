...
class AsyncExoTaskModule(TaskModule):
    ...
    def run_task(self):
        ...
        self.wait(5)
        self.state = self.STATE_EEG_SENS
        self.entered_state = clock()
        logger.info(f"Changed state to {self.state} {self.STATE_NAMES[self.state]}")
        self.wait(60*120)
    ...
    def process_data(self, sample, timestamp):
        ...
        # close exo state
        elif self.state == self.STATE_SEND_CLOSE:

            # if exo had time to close for 2s, stop sending close command and switch state
            if clock()-self.entered_state >= self.getParameter("send_close_length"):

                self.state = self.STATE_EEG_EOG_SENS
                self.entered_state = clock()
                logger.info(f"Change state to {self.state} {self.STATE_NAMES[self.state]}")

                self.left_exo_state = ExoState.STOP
                self.right_exo_state = ExoState.STOP

        # open exo state
        elif self.state == self.STATE_SEND_OPEN:

            # if exo had time to open for 2s, stop sending open command and switch state
            if clock()-self.entered_state >= self.getParameter("send_open_length"):
        ...
        elif self.state == self.STATE_SEND_FASTEN:

            # if exo had time to close further for 1s, stop sending open command and switch state
            if clock()-self.entered_state >= self.getParameter("send_fasten_length"):
        ...
        # exo is open and resting state
        elif self.state == self.STATE_EEG_SENS:
            ...
            # if ERD is ongoing
                ...
                if clock()-self.eeg_start >= self.getParameter("close_erd_length"):
                    ...
                    self.state = self.STATE_SEND_CLOSE
                    self.entered_state = clock()
                    logger.info(f"Change state to {self.state} {self.STATE_NAMES[self.state]}")

                    if Side(self.getParameter("exo_side")) is Side.RIGHT:
                        self.right_exo_state = ExoState.CLOSE
                    elif Side(self.getParameter("exo_side")) is Side.LEFT:
        ...
        # exo is open and resting state
        elif self.state == self.STATE_EEG_EOG_SENS:
            ...
            if self.HOV:
                ...
                self.state = self.STATE_SEND_OPEN
                self.entered_state = clock()
                logger.info(f"Change state to {self.state} {self.STATE_NAMES[self.state]}")
            ...
            # if ERD is ongoing
                ...
                if clock()-self.eeg_start >= self.getParameter("fasten_erd_length"):

                    self.state = self.STATE_SEND_FASTEN
                    self.entered_state = clock()
                    logger.info(f"Change state to {self.state} {self.STATE_NAMES[self.state]}")
    ...
    def onStop(self):
        ...
        if self.state == self.STATE_SEND_CLOSE or self.state == self.STATE_SEND_FASTEN or self.state == self.STATE_EEG_EOG_SENS:
