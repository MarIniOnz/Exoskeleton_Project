...
class ColorCircleWidget(QWidget):
    ...
    def set_color(self, r: int, g: int, b: int):
        ...
        self.color = Qt.QtGui.QColor(r, g, b)
...
class SinglePacmanFeedbackApp(QMainWindow):
    ...
    def __init__(self, left, top, width=1000, height=700, fullscreen=False, maximized=False, frameless=False, side=Side.RIGHT, display_pacman: bool = True,  display_relax: bool = False):
        ...
        # create color circle widget
        self.color_circle = ColorCircleWidget()
        ...
        lay.addWidget(self.color_circle, 2, 3, 4, 4)
    ...
    def data_handler(self):
        ...
        while True:
            ...
            if sample is not None:
                ...
                if len(str(sample[0])) == 10 and str(sample[0])[0] == '1':

                    color_r = int(str(sample[0])[1:4])
                    color_g = int(str(sample[0])[4:7])
                    color_b = int(str(sample[0])[7:10])
                    self.color_circle.set_color(color_r, color_g, color_b)

                if not (int(sample[0]) == Cue.RELAX.value and self.display_relax == False) and int(sample[0]) in DisplayText.keys():
...
if __name__ == "__main__":
    ...
    """
    # Search in command line arguments for which pacman (left/right hand) to display
    side = None
    for a in sys.argv:
        if a.strip().upper() == "SIDE=LEFT":
            side = Side.LEFT
        elif a.strip().upper() == "SIDE=RIGHT":
            side = Side.RIGHT
    
    # if no pacman was selected, quit.
    if side is None:
        print("No active hand selected - add 'SIDE=left/right' to invocation.")
        exit()

    
    # search in command line arguments for wether pacman is to be shown or not
    show_pacman = None
    for a in sys.argv:
        if a.strip().upper() == "SHOWPACMAN=TRUE":
            show_pacman = True
        elif a.strip().upper() == "SHOWPACMAN=FALSE":
            show_pacman = False

    if show_pacman is None:
        print("Wether to display pacman was not selected - add 'SHOWPACMAN=true/false' to invocation.")
        exit()

    show_open = None
    for a in sys.argv:
        if a.strip().upper() == "SHOWOPEN=TRUE":
            show_open = True
        elif a.strip().upper() == "SHOWOPEN=FALSE":
            show_open = False

    if show_open is None:
        print("Whether to display pacman when opening was not selected - add 'SHOWOPEN=true/false' to invocation.")
        exit()
    """
