...
class EOGCalibrationTaskModule(TaskModule):
    ...
    def run_task(self):
        ...
        if self.getParameter('directions') == self.DIRECTIONS_ONLY_LEFT:
            cues = [Cue.HOVLEFT]
        elif self.getParameter('directions') == self.DIRECTIONS_ONLY_RIGHT:
            cues = [Cue.HOVRIGHT]
        ...
        for c in cues:
            ...
            self.cue = c
            self.wait(self.getParameter('cue_length'))

            # do ITI: wait for   iti_min <= wait-time <= iti_max   seconds
            self.cue = Cue.EMPTY
            self.wait(self.getParameter('iti_min') + random.random() * (self.getParameter('iti_max') - self.getParameter('iti_min') ))
